...
bool PowerManager::initialize()
{
    ...
    if (!m_conn->registerObject(kPath, this,
        ...
        qWarning("Power::Session: registerObject failed");
        return false;
    }
...
{
    qWarning("[Power::Mgr] PSM enabled signal received: %d", enabled);
    if (m_powerSavePlan)
...
{
    qWarning("[Power::Mgr] PSM brightness drop signal received: %u", value);
    if (m_powerSavePlan)
...
{
    qWarning() << "=====[prepare suspend]======" << static_cast<int>(m_prepareSuspendState) << m_sleepLock;
...
{
    qWarning() << "=====[can suspend]======" << canSuspend();
    if (!canSuspend())
        return;
    qWarning() << "=====[can suspend1]======" << canSuspend();
...
void PowerManager::recalculateScheduledShutdown()
{
    ...
    qWarning() << m_nextShutdownTime << QDateTime::fromSecsSinceEpoch(m_nextShutdownTime).toString("yyyy-MM-dd hh:mm:ss");
    if (m_config)
...
void PowerManager::initDConfig()
{
    ...
    // ── DConfig bindings: { key, apply, onChange? } ──
    //   apply:   called on initial load AND on every runtime change
    //   onChange: called AFTER apply, only on runtime changes (optional)
    //
    //   To add a new key: insert one line below.
    //   If the key needs side-effect logic on change, provide an onChange callback.
    ...
    std::vector<Binding> bindings = {
        ...
        { PowerDConfig::kScheduledShutdownState,
          ...
          [this]() {
              if (m_scheduledShutdownState) {
                  qWarning("[Power::Sched] DConfig: state=ON, recalculating");
                  recalculateScheduledShutdown();
              } else {
                  qWarning("[Power::Sched] DConfig: state=OFF, canceling");
        ...
        { PowerDConfig::kShutdownTime,
          ...
          [this]() {
              if (m_scheduledShutdownState) {
                  qWarning("[Power::Sched] DConfig: time=%s state=on, recalculating",
                           qPrintable(m_shutdownTime));
                  recalculateScheduledShutdown();
              }
          }
        },
        { PowerDConfig::kShutdownRepetition,
          ...
          [this]() {
              if (m_scheduledShutdownState) {
                  qWarning("[Power::Sched] DConfig: repetition=%d state=on, recalculating",
                           m_shutdownRepetition);
                  recalculateScheduledShutdown();
              }
          }
        },
        { PowerDConfig::kCustomShutdownWeekDays,
          ...
          [this]() {
              if (m_scheduledShutdownState) {
                  qWarning("[Power::Sched] DConfig: weekDays changed, state=on, recalculating");
                  recalculateScheduledShutdown();
              }
          }
        },
    ...
    connect(m_config, &DConfig::valueChanged, this,
        ...
        qWarning("[Power::DConfig] changed: %s = %s", qPrintable(k), qPrintable(v.toString()));
    ...
    qWarning("[Power::DConfig] initial load: ss=%d black=%d sleep=%d lock=%d",
             m_linePowerScreensaverDelay, m_linePowerScreenBlackDelay,
             m_linePowerSleepDelay, m_linePowerLockDelay);
}
...
void PowerManager::onNotifyActionInvoked(uint id, const QString &actionKey)
{
    ...
    qWarning("[Power::ScheduledShutdown] action invoked: %s", qPrintable(actionKey));
...
void PowerManager::initScheduledShutdown()
{
    ...
    qWarning("[Power::ScheduledShutdown] init: state=%d time=%s rep=%d countdown=%d",
             (int)m_scheduledShutdownState,
             qPrintable(m_shutdownTime), m_shutdownRepetition, m_shutdownCountdown);
    if (m_scheduledShutdownState) {
...
void PowerManager::onSystemTimeChanged()
{
    ...
    qWarning("[Power::ScheduledShutdown] system time changed, recalculating");
    m_nextShutdownTime = getNextShutdownTime(0);
    if (m_config) m_config->setValue(kNextShutdownTime, m_nextShutdownTime);
    scheduledShutdown(SchedInit);
}
...
{
    if (!m_scheduledShutdownState) return;
    qWarning("[Power::ScheduledShutdown] session active changed, re-trigger");
    scheduledShutdown(SchedInit);
}
...
{
    closeNotify();
    qWarning("[Power::ScheduledShutdown] executing shutdown");
    m_proxy->requestShutdown();
}
...
{
    qWarning("[Power::ScheduledShutdown] pre=%d next=%d nextTime=%s state=%d time=%s rep=%d cnt=%d",
             m_shutdownStatus, state,
             qPrintable(QDateTime::fromSecsSinceEpoch(m_nextShutdownTime).toString("yyyy-MM-dd hh:mm:ss")),
             (int)m_scheduledShutdownState,
             qPrintable(m_shutdownTime), m_shutdownRepetition, m_shutdownCountdown);
    ...
    if (!isSessionActive || m_warnLevel == LowPowerManager::Action) {
        closeNotify();
        qWarning("[Power::ScheduledShutdown] Stop: isSessionActive=%d warnLevel=%d",
                 (int)isSessionActive, (int)m_warnLevel);
        return;
    }

    if (!m_scheduledShutdownState && state == SchedInit) {
        qWarning("[Power::ScheduledShutdown] state=off, skipping SchedInit");
        return;
    }

    if (state != SchedInit && state == m_shutdownStatus) {
        qWarning("[Power::ScheduledShutdown] same status=%d, skip", state);
        return;
    }

    if (m_nextShutdownTime == 0) {
        qWarning("[Power::ScheduledShutdown] nextTime=0, skip");
        return;
    }
    ...
    case SchedInit: {
        ...
        qWarning("[Power::ScheduledShutdown] SchedInit: diff=%lldmin countdown=%d → next=%d",
                 diffMins, m_shutdownCountdown, nextStatus);
        scheduledShutdown(nextStatus);
        break;
    }
    case SchedWaitingToNotify: {
        ...
        qWarning("[Power::ScheduledShutdown] WaitingToNotify: timer=%lldms (notifyAt=%s)",
                 msToNotify, qPrintable(notifyAt.toString("yyyy-MM-dd hh:mm:ss")));
        QObject::disconnect(m_shutdownTimer, &QTimer::timeout, this, nullptr);
    ...
    case SchedCountdowning: {
        ...
        QObject::disconnect(m_countdownTimer, &QTimer::timeout, this, nullptr);
    ...
    case SchedCancel:
    case SchedTimeout: {
        qWarning("[Power::ScheduledShutdown] terminal state=%d (Shutdown=3 Cancel=4 Timeout=5)", state);
        if (state == SchedTimeout) closeNotify();
        m_shutdownStatus = state;

        if (m_shutdownRepetition == RepOnce) {
            qWarning("[Power::ScheduledShutdown] rep=Once, disabling");
            ...
            qWarning("[Power::ScheduledShutdown] rep=%d, computing next time from base=%lld",
                     m_shutdownRepetition, m_nextShutdownTime);
            m_nextShutdownTime = getNextShutdownTime(m_nextShutdownTime);
            if (m_config) m_config->setValue(kNextShutdownTime, m_nextShutdownTime);

            qint64 t = now.secsTo(next) / 60;
            if (t > 0) {
                qWarning("[Power::ScheduledShutdown] rep schedule: delay=10s (in time, %lldmin)", t);
                QTimer::singleShot(10000, this, [this]() { scheduledShutdown(SchedInit); });
            } else {
                qWarning("[Power::ScheduledShutdown] rep schedule: delay=%ds (past time)", m_shutdownCountdown);
...
bool PowerManager::isCustomDay(const QDateTime &date) const
{
    ...
    for (int i = 0; i < m_customShutdownWeekDays.size(); ++i) {
        if (m_customShutdownWeekDays[i] == dow) return true;
    }
    return false;
}
...
qint64 PowerManager::getNextShutdownTime(qint64 baseTime) const
{
    ...
    qWarning("[Power::ScheduledShutdown] next shutdown time: %s",
             qPrintable(targetTime.toString("yyyy-MM-dd hh:mm:ss")));
