...
IdleNotifier::getIdleNotification(uint32_t timeout, wl_seat *seat)
{
    ...
    if (!raw) {
        qWarning() << "[Power::IDLE] getIdleNotification: failed to create idle notification";
        return nullptr;
    }
    ...
    : IdleWatcher(parent)
{
    auto *app = qGuiApp;
    if (!app) {
        qWarning("[Power::WL] Idle: no QGuiApplication");
        return;
    }
    ...
    if (!wlApp) {
        qWarning("[Power::WL] Idle: no QWaylandApplication");
        return;
    }

    m_seat = wlApp->seat();
    if (!m_seat) {
        qWarning("[Power::WL] Idle: no wl_seat");
        return;
    }
    ...
    if (!m_notifier->isInitialized()) {
        qWarning("[Power::WL] Idle: notifier init failed");
        return;
    }
...
void WaylandIdleWatcher::switchToNotification(uint32_t timeoutSec)
{
    ...
    if (it == m_notificationCache.end()) {
        ...
        if (!notif) {
            qWarning() << "[Power::IDLE] switchToNotification: failed, timeout="
                       << timeoutSec << "s";
            return;
        }
        qWarning() << "[Power::IDLE] create & cache notification for timeout=" << timeoutSec << "s";
        auto [inserted, _] = m_notificationCache.emplace(timeoutSec, std::move(notif));
        it = inserted;
    } else {
        qWarning() << "[Power::IDLE] reuse cached notification timeout=" << timeoutSec << "s";
    }
    ...
    connect(m_activeNotification, &IdleNotification::idled, this, [this]() {
        ...
        qWarning() << "[Power::IDLE] >>> IDLED (timeout=" << m_timeoutSec << "s) <<<";
...
void WaylandIdleWatcher::setTimeout(uint32_t s)
{
    ...
    qWarning() << "[Power::IDLE] setTimeout =" << s << "s";
...
{
    qWarning() << "[Power::IDLE] simulateActivity (no-op in Wayland idle protocol)";
}
