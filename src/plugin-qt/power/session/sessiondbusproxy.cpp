          ...
          QDBusConnection::systemBus(), this))
{
    ...
    QDBusConnection::sessionBus().connect(
        ...
        "PropertiesChanged", this, SIGNAL(timeUpdate()));
...
{
    bool v = m_powerInter->property("PowerSavingModeEnabled").toBool();
    qWarning("[Proxy] powerSavingModeEnabled() = %d", v);
    return v;
}

uint SessionDBusProxy::powerSavingModeBrightnessDropPercent() const
{
    uint v = m_powerInter->property("PowerSavingModeBrightnessDropPercent").toUInt();
    qWarning("[Proxy] powerSavingModeBrightnessDropPercent() = %u", v);
    return v;
}
