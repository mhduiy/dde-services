    ...
    for (const auto &entry : ps.entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
        ...
        if (sf.open(QIODevice::ReadOnly)) {
            ...
            if (s == "Charging") status = 1;
                ...
                tte = pw > 0 ? readInt("energy_now") / pw * 3600 : 0; }
...
void BatteryManager::initUdev()
{
    m_udev = udev_new();
    if (!m_udev) {
        qWarning("[Power::Sys] udev_new failed");
        return;
    }

    m_udevMon = udev_monitor_new_from_netlink(m_udev, "udev");
    if (!m_udevMon) {
        qWarning("[Power::Sys] udev_monitor_new_from_netlink failed");
        return;
    }

    if (udev_monitor_filter_add_match_subsystem_devtype(m_udevMon, "power_supply", nullptr) < 0) {
        qWarning("[Power::Sys] udev_monitor_filter_add_match failed");
        return;
    }

    if (udev_monitor_enable_receiving(m_udevMon) < 0) {
        qWarning("[Power::Sys] udev_monitor_enable_receiving failed");
        return;
    }

    int fd = udev_monitor_get_fd(m_udevMon);
    if (fd < 0) {
        qWarning("[Power::Sys] udev_monitor_get_fd failed");
        return;
    }
    ...
    qWarning("[Power::Sys] udev monitor ready for power_supply");
}
